...
//	        huobiAccount.Trader = new HuobiTrader();
                 ...
                 (currentTime > PreviousStrategy?.TradeLastTime.AddSeconds(InputParameters.TradeLagThreshold))))
            {
                if (differPrice > TradeThreshold)
                {
                    if (btccPrice > huobiPrice) //btcc卖价高
                    {
                        ...
                        {
                            sellSucceeded = btccAccount.Sell(Id, btccPrice, tradeAmount, guid, Form4.BtccPrefix);
                            //btcc卖出
                            buySucceeded = huobiAccount.Buy(Id, huobiPrice, tradeAmount, guid, Form4.HuobiPrefix);
                            //huobi买入
                        }
                    }
                    else //huobi卖价高
                    {
                        ...
                        {
                            sellSucceeded = huobiAccount.Sell(Id, huobiPrice, tradeAmount, guid, Form4.HuobiPrefix);
                            //huobi卖出
                            buySucceeded = btccAccount.Buy(Id, btccPrice, tradeAmount, guid, Form4.BtccPrefix); //btcc买入
                        }
                    }
                }
            }
            ...
            if (differPrice < RegressionThreshold)
            {
                if (btccAccount.CoinAmount > huobiAccount.CoinAmount) //btcc币多
                {
                    if (btccAccount.CoinAmount != 0)
                    {
                        sellSucceeded = btccAccount.Sell(Id, btccPrice, tradeAmount, guid, Form4.BtccPrefix); //btcc卖出
                        buySucceeded = huobiAccount.Buy(Id, huobiPrice, tradeAmount, guid, Form4.HuobiPrefix); //huobi买入
                    }
                }
                else //huobi币多
                {
