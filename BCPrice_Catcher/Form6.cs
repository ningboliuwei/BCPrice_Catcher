...
//            {
                if (_accounts[_outSiteCode].AccountTradeRecords.Count != 0)
                {
                    for(int i =0; i < _accounts[_outSiteCode].AccountTradeRecords.Count;i++)
                    {
                        ...
                        {
                            _accounts[_outSiteCode].RealPlacedOrders.Add(
                                    _accounts[_outSiteCode].Trader?.GetPlacedOrder(t.OrderId,Trader.Trader.CoinType.Btc));
                        ...
                        else
                        {
                            ...
                            {
                                _accounts[_outSiteCode].RealPlacedOrders.Add(
                                    _accounts[_outSiteCode].Trader?.GetPlacedOrder(t.OrderId,
                        ...
                        for (int i = 0; i < notClosedRealPlacedOrder.Count; i++)
                        {
                            if (notClosedRealPlacedOrder[i] != null)
                            {
                                notClosedRealPlacedOrder[i] =
                                    _accounts[_outSiteCode].Trader?.GetPlacedOrder(notClosedRealPlacedOrder[i].Id,
                                        Trader.Trader.CoinType.Btc);
                            }
                        }
                    }
                ...
                if (_accounts[_inSiteCode].AccountTradeRecords.Count != 0)
                {
                    for (int i = 0; i < _accounts[_inSiteCode].AccountTradeRecords.Count; i++)
                    {
                        ...
                        {
                            _accounts[_inSiteCode].RealPlacedOrders.Add(
                                _accounts[_inSiteCode].Trader?.GetPlacedOrder(t.OrderId,
                                    Trader.Trader.CoinType.Btc));
                        }
                        else
                        {
