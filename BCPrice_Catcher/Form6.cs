...
namespace BCPrice_Catcher
{
	...
	public partial class Form6 : Form
	{
		...
		private int _strategyInterval = 1000;
		...
		{
			StrategyTimerList.Timers["strategy"].Change(0, _strategyInterval);
			...
			StrategyTimerList.Add("auto_buy", Timeout.Infinite, o =>
			{
				try
				{
					if (_inRealMode)
					{
						...
						if (DateTime.Now.Subtract(_noCoinStartTime).Seconds >= Strategies[0].InputParameters.AutoBuyLag &&
							_allowAutoBuy)
						{
							int orderId;
							if (_prices[_outSiteCode] <= _prices[_inSiteCode])
							{
								...
								var amountToBuy = Math.Round(_accounts[_outSiteCode].Balance *
															 (Strategies[0].InputParameters.AutoBuyBalancePercentage /
															  100.0) /
															 _prices[_outSiteCode], 3);

								orderId =
									_accounts[_outSiteCode].Trader.Buy(
										_prices[_outSiteCode] + strategy.InputParameters.b, amountToBuy,
							...
							else
							{
								...
								orderId =
									_accounts[_inSiteCode].Trader.Buy(
										_prices[_inSiteCode] + strategy.InputParameters.b, amountToBuy,
										Trader.Trader.CoinType.Btc);
								//insite buy
							}
		...
		private void ShowPrices()
		{
			...
			lblDifferPrice.Text
				= $"当前差价：{_baseDifferPrice.ToString("0.000")}{Environment.NewLine}最近{nudAutoTrackPeroid.Text}秒平均差价：{_averageBaseDifferPrice.ToString("0.000")}";
		...
		private void CaculateAverageDifferPrice()
		{
			#region Cacualte average differ price

			_historyDifferPrices.Enqueue(_baseDifferPrice);

			int maxHistoryCount = Convert.ToInt32(nudAutoTrackPeroid.Text);
			if (_historyDifferPrices.Count > maxHistoryCount)
			{
				int count = _historyDifferPrices.Count;
				for (int i = 0; i < count - maxHistoryCount; i++)
				{
		...
		private void nudStrategyInterval_ValueChanged(object sender, EventArgs e)
		{
			const string integerRegex = @"^(\+|-)?\d+$";
			var s11 = nudStrategyInterval.Text;

			_strategyInterval = Regex.IsMatch(s11, integerRegex) ? Convert.ToInt32(s11) : 1;
			StrategyTimerList.Timers["strategy"].Change(0, _strategyInterval);
		}
	}
}